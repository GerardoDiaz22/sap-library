sap.ui.define([
    ...
    function (Controller, MessageToast) {
        ...
        return Controller.extend("sap.lib.update.updatebooks.controller.UpdateBook", {
            onInit: function () {

            },
            onUpdatePress: async function () {
                /* Books */
                const ID = this.getView().byId("bookID")
                    .getValue()
                    .trim();
                ...
                if (title === "" || authors === ""){
                    ...
                    await $.ajax({
                        url: `http://localhost:4004/service/books/Books(${ID})`,
                        method: 'PATCH',
                        contentType: "application/json",
                        data: JSON.stringify({
                            "title": title,
                ...
                /* BookAuthors */
                await $.ajax({
                    url: `http://localhost:4004/service/books/BooksAuthors(${ID})`,
                    method: 'DELETE'
                });
                authors_IDs
                ...
                /* BookCategories */
                await $.ajax({
                    url: `http://localhost:4004/service/books/BooksCategories(${ID})`,
                    method: 'DELETE'
                });
                categories_IDs
